...
module.exports = {
	...
	async getRepository(options) {
		...
		const repoInfo = await getJSON(`https://api.bitbucket.org/2.0/repositories/${repoPath}`);
		...
		if (!repoInfo.error) {
			...
			const currentProject = await getJSON(
				`https://api.bitbucket.org/2.0/repositories/${env('BITBUCKET_REPO_OWNER')}/${env('BITBUCKET_REPO_SLUG')}`
			);
			...
			const newProjectResult = await getJSON(`https://api.bitbucket.org/2.0/repositories/${repoPath}`, {
				method: 'POST',
